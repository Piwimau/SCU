...
int8_t scu_random_int8(SCURandom* random, int8_t min, int8_t max) {
    ...
    // Strictly speaking, this is not necessary for small types like `int8_t` and `int16_t`,
    // since they are promoted to larger types anyway by the usual integer promotion rules, and the
    // range should be sufficient. However, it is not wrong and so we implement it consistently for
    // all signed integer variants.
    ...
    uint8_t threshold = -range % range;
    uint8_t result;
    do {
        // We simply extract the required 8 bits from the top of the large pseudorandom `uint64_t`
        // generated by `scu_next()`.
        result = scu_next(random) >> 56;
    }
...
int16_t scu_random_int16(SCURandom* random, int16_t min, int16_t max) {
    ...
    // that would otherwise be introduced by the modulo operator. Typically, very few iterations are
    // required to find a suitable pseudorandom number.
    uint16_t threshold = -range % range;
    uint16_t result;
    do {
        // We simply extract the required 16 bits from the top of the large pseudorandom `uint64_t`
        // generated by `scu_next()`.
        result = scu_next(random) >> 48;
    }
    while (result < threshold);
    return min + (result % range);
}
