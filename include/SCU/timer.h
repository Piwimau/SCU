 ...
 *
 * @note If `timer` has never been started before or was reset with `scu_timer_reset()`,
 * this starts a new time interval. Otherwise, it resumes measuring time in the current interval.
 *
 ...
 * time of the current interval. Otherwise, if the `SCUTimer` is still running, the elapsed CPU
 ...
 *
 * @note The elapsed wall time is based on the current state of the `SCUTimer`. If it has never
 * been started before or was reset using `scu_timer_reset()`, the elapsed wall time is zero, as no
 * time interval has been recorded yet. If the `SCUTimer` was previously stopped using
 * `scu_timer_stop()`, the elapsed wall time corresponds to the difference between the start and
 * end time of the current interval. Otherwise, if the `SCUTimer` is still running, the elapsed
 * wall time is calculated as the difference between the start time of the current interval and the
 * current point in time.
 *
